...
*       gs_layout-coltab_fieldname  = 'COLOR'.
  ...
  METHOD top_of_header.
    ...
    ls_top-typ  = 'H'                          .
    IF rb1 EQ 'X'.
      ls_top-info = 'Taşeron İşe Alım Batch'   .
    ELSE.
      ls_top-info = 'Taşeron İşten Çıkış Batch'.
    ENDIF.
  ...
  METHOD fill_fcat.
    ...
    IF rb1 EQ 'X'.
      lv_tabname = 'ZNCPRH_S016'.
    ELSE.
      lv_tabname = 'ZNCPRH_S017'.
    ENDIF.
  ...
  METHOD modify_fcat.
    LOOP AT gt_fcat ASSIGNING FIELD-SYMBOL(<fs_fcat>).
      ...
      CASE <fs_fcat>-fieldname.
        ...
        WHEN 'ICON'.
          <fs_fcat>-icon = 'X'.
          <fs_fcat>-seltext_l = <fs_fcat>-seltext_m =
          <fs_fcat>-seltext_s = <fs_fcat>-reptext_ddic =
          'Ikon'.
        WHEN OTHERS.
      ENDCASE.
    ENDLOOP.
...
*      MESSAGE TEXT-003 TYPE 'I' DISPLAY LIKE 'E'.
    ...
    ELSE.
      ...
      lv_def_name = 'isten_cikis_Batch.xls'.
      CONCATENATE 'Personel Numarası'
                'Çıkış Tarihi'
                'Çıkış Nedeni'
      INTO file-field SEPARATED BY con_tab .
      APPEND file TO file.

      CONCATENATE '00001362'
                  '01.01.2021'
                  '18'
               INTO file-field SEPARATED BY con_tab .
      APPEND file TO file.
...
*        '0001' 'P0001-PLANS' <fs_data>-plans,
        '0001' 'P0001-ABKRS' <fs_data>-abkrs,
*        '0001' 'P0001-PERSG' <fs_data>-persg,
*        '0001' 'P0001-PERSK' <fs_data>-persk,
*        '0001' 'P0001-SBMOD' <fs_data>-sbmod,
*-
...
*-
        '9050' 'P9050-FKODU' <fs_data>-fkodu,
        '9050' 'P9050-IKODU' <fs_data>-ikodu.
